...
final class MenuItemsData extends Data
{
  ...
  public static function fromModel(MenuItem $menuItem): self
  {
    return new self(
      ...
      childs: Lazy::whenLoaded('childs', $menuItem,
        static fn() => MenuItemsData::collection($menuItem->childs)),
      menu: Lazy::whenLoaded('menu', $menuItem,
        fn() => MenuData::collection($menuItem->menu))
    );
  }
   ...
   * @return Collection
   */
  public static function fromData(Collection $attributes): Collection
  {
    return $attributes->map(function ($item) {
      return new self(
        ...
        menu_id: Optional::create(),
        childs: Lazy::when(static fn() => !empty(data_get($item, 'childs')),
          static fn() => self::collection(
            items: $item['childs']->map(function ($value) {
              return [
                'id' => $value->id,
                'label' => $value->label,
                'route' => $value->route
