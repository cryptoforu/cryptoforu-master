...
final readonly class StorePageMeta
{
  ...
  public function handle(StorePageRequest $from): void
  {
    ...
    $image_paths = config('app.url').'/img/cache/original/6453f5dad4e11.png';
    ...
    if ($from->hasFile('meta_image')) {
      ...
      $meta_path = config('app.url').'/'.$meta_image['path'];
    }
    if ($from->hasFile('tw_image')) {
      ...
      $tw_path = config('app.url').'/'.$tw_image['path'];
    }
    if ($from->hasFile('og_image')) {
      ...
      $og_path = config('app.url').'/'.$og_image['path'];
    }
    ...
    if (!empty($from->allFiles())) {
      collect([
        $meta_path ?? $image_paths, $tw_image ?? $image_paths,
        $og_image ?? $image_paths
      ])
