...
final class Kernel extends ConsoleKernel
{
  ...
  protected function schedule(Schedule $schedule): void
  {
    ...
    $schedule->command('app:update-faucet-list')->everyFifteenMinutes(
    )->runInBackground();
    $schedule->command('telescope:prune')->daily()->runInBackground();
    $schedule->command('app:update-crypto-categories')->weekly(
    )->runInBackground();
    $schedule->command('app:process-crypto-exchanges')->weekly(
    )->runInBackground();
    $schedule->command('app:write-ziggy')->monthly()->runInBackground();
  }
